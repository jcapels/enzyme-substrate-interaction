...
class MolecularGAT3D(pl.LightningModule):
    def __init__(self, in_dim, hidden_dim, num_heads_gat, num_heads_attention, dropout_gat=0.1, activation_attention = nn.ReLU(), 
        ...
        self.cross_attention_block_descriptors = AttentionBlock(hidden_dim, num_heads_attention, dropout_attention)

        self.mol_descriptors_batch_norm = nn.BatchNorm1d(849)
        self.protein_descriptors_batch_norm = nn.BatchNorm1d(1106)
        self.ffn_protein_descriptors = nn.Sequential(nn.Linear(1106, hidden_dim),
                                                 nn.ReLU(),
                                                 nn.BatchNorm1d(hidden_dim))
        self.ffn_mol_descriptors = nn.Sequential(nn.Linear(849, hidden_dim),
                                                 nn.ReLU(),
                                                 nn.BatchNorm1d(hidden_dim))
    ...
    def forward(self, data):
        ...
        if self.use_descriptors:
            ...
            mol_descriptors = self.ffn_mol_descriptors(mol_descriptors)
            protein_descriptors = data.protein_descriptors.reshape(batch_size, 1106)
            protein_descriptors = self.protein_descriptors_batch_norm(protein_descriptors)
            protein_descriptors = self.ffn_protein_descriptors(protein_descriptors)
        ...
        if self.use_descriptors:
            x = torch.concat((x, x_descriptors), dim=1)
...
class MolecularGraphDataset(Dataset):
    def __init__(self, mol_dict, mol_descriptors, protein_descriptors, proteins_embeddings_dict, interactions, labels,
        ...
        for key, mol in self.mol_dict.items():
            ...
            data.mol_descriptors = torch.tensor(self.mol_descriptors[key], dtype=torch.float32)
