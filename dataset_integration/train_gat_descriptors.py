...
import numpy as np
from enzyme_substrate_prediction.models.structural_gat_with_descriptors  import MolecularGAT3D, MolecularGraphDataset, collate_fn
...
class BindingExperiment(Experiment):
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        for train_dataset, validation_dataset, test_dataset in datasets:
            try:
                model = train_model(train_dataset, validation_dataset, hidden_dim, num_heads_gat,
                                    num_heads_attention, activation_attention, dropout_gat,
                                    use_descriptors, prediction_layers, dropout_attention,
        ...
        return np.mean(f1_macro_validation)
...
def experiment_optimize(ids_for_datasets):
    ...
    experiment = BindingExperiment(ids_for_datasets=ids_for_datasets, study_name="binding_3d_gat", storage="sqlite:///binding_3d_gat.db", sampler= optuna.samplers.TPESampler(seed=123),
                                    direction="maximize", load_if_exists=True, results_output_file="binding_3d_gat.csv",
...
def test_train_model():
    ...
    for train_dataset, validation_dataset, test_dataset in datasets:
        try:
            model = train_model(train_dataset, validation_dataset, 2048, 1,
                                2, nn.ReLU(), 0.5,
                                True, [2048, 512], 0.3,
