...
def load_datasets(train_dataset, test_dataset, random_state=42, merge_validation_set=False):
    import pandas as pd
    ...
    if not merge_validation_set:
        ...
        for i in range(5):
            ...
            train_dataset_ = train_dataset[train_dataset[f"kfold_{i}"] == "train"]
            train_dataset_ = train_dataset_.sample(frac=1, random_state=random_state).reset_index(drop=True)
            
            import pandas as pd

            validation_dataset__ = train_dataset[train_dataset[f"kfold_{i}"] == "valid"]
...
def test_train_model(train_dataset, test_dataset, protein_head_layers, compound_head_layers, final_head_layers, batch_norm, dropout, batch_size, learning_rate, epochs):
    ...
    for i in range(5):
        ...
        train_dataset_, test_dataset_ = load_datasets(train_dataset_, test_dataset_, random_state=i, merge_validation_set=True)

        # Cross-validation loop
        model, _ = get_model(module, batch_size, epochs, train_dataset_, None)
        
        # Evaluate on test set
        predictions_probability = model.predict_proba(test_dataset_)
        ...
        df.to_csv("results_np_classifier.csv", mode='a', index=False, header=not file_exists)
        file_exists = True  # Ensure header is not written again
    
def experiment_optimize(train_dataset, test_dataset):
    
    experiment = BindingExperiment(train_dataset, test_dataset, study_name="binding_np_classifier", storage="sqlite:///binding_np_classifier.db", sampler= optuna.samplers.TPESampler(seed=123),
    ...
    test_train_model(train_dataset, test_dataset, experiment.best_hyperparameters["protein_head_layers"], experiment.best_hyperparameters["compound_head_layers"],
...
def experiment_optimize_v2(train_dataset, validation_dataset, test_dataset):
    ...
    experiment = BindingExperiment(train_dataset, validation_dataset, test_dataset, study_name="binding_np_classifier", storage="sqlite:///binding_np_classifier.db", sampler= optuna.samplers.TPESampler(seed=123),
                                    direction="maximize", load_if_exists=True, results_output_file="binding_np_classifier.csv",
                                        folder_path="kroll_experiment_models")
    experiment.run(n_trials=50, n_jobs=1)
...
if __name__ == "__main__":
    ...
    train_dataset = pd.concat([train_dataset, validation_dataset], ignore_index=True)
    test_train_model(train_dataset, test_dataset, [2560, 1280, 1280, 640], [2560, 1280],[2560, 2560, 2560, 1280], True, 0.4666248594168294, 32, 6.196659755586358e-05, 24)
