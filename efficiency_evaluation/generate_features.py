...
def process_to_esm2(multi_input_dataset, output_path):
    ...
    multi_input_dataset.save_features(output_path)

def process_probert(multi_input_dataset, output_path):
    from plants_sm.featurization.proteins.bio_embeddings.prot_bert import ProtBert
    transformer = ProtBert(batch_size=1, device="cuda:1")

    multi_input_dataset = transformer.fit_transform(multi_input_dataset, "proteins")

    multi_input_dataset.save_features(output_path)

def process_esm1b(multi_input_dataset, output_path):
    from plants_sm.featurization.proteins.bio_embeddings.esm import ESMEncoder
    transformer = ESMEncoder(esm_function="esm1b_t33_650M_UR50S", batch_size=1, device="cuda:1")

    multi_input_dataset = transformer.fit_transform(multi_input_dataset, "proteins")

    multi_input_dataset.save_features(output_path)

def generate_features_for_compounds(multi_input_dataset, output_path):
...
if __name__ == "__main__":
    ...
    process_to_esm2(multi_input_dataset)
