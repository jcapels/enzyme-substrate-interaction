...
train_csv = "/Users/anamonteiro/Downloads/train_dataset_80_similarity_threshold.csv"
test_csv = "/Users/anamonteiro/Downloads/test_dataset_80_similarity_threshold.csv"
...
def get_unique_chemicals(csv_path, column="SMILES"):
    df = pd.read_csv(csv_path)
...
dataset_df = pd.read_csv("curated_dataset_no_stereochemistry_duplicates.csv")

train_df = pd.read_csv(train_csv)
test_df = pd.read_csv(test_csv)

train_seq_unique = train_df[[SEQUENCE_COL]].drop_duplicates().reset_index(drop=True)
test_seq_unique = test_df[[SEQUENCE_COL]].drop_duplicates().reset_index(drop=True)
...
train_smiles_unique = pd.DataFrame({SMILES_COL: get_unique_chemicals(train_csv, SMILES_COL)})
test_smiles_unique = pd.DataFrame({SMILES_COL: get_unique_chemicals(test_csv, SMILES_COL)})
...
for _, test_row in test_df.iterrows():
    ...
    test_substrate_id = train_smiles_to_id.get(test_ligand,
                                               f"S_temp_{test_ligand}")


    hits = blast_df[blast_df["query_id"] == test_enzyme_id].sort_values("bitscore", ascending=False)

    blast_train_ids = hits["subject_id"].tolist()
    blast_scores = hits["bitscore"].tolist()
